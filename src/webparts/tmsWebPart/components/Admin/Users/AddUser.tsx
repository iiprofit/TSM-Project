...
class AddUser extends React.Component<IAddUserProp, IAddUserStates> {
  ...
  fetchUserEmails = async () => {
    try {
      ...
      const _data = result.value.length
        ? result.value
            .filter(
              (x) => !_usersInSystem.some((y) => y.Email.EMail === x.Email)
            )
            .map((x) => ({
  ...
  fetchUsersFromAD = async (userQuery) => {
    ...
    const _usersInTenant = await Promise.all(responseArr.map((x) => x.json()));
    ...
    // ! I need to know more about this code
    response = await httpClient.get(params.url, params.config, params.options);
    result = await response.json();
    const _usersInSystem = result.value;
    const _data = _usersInTenant.length
      ? _usersInTenant
          .filter((x) => !_usersInSystem.some((y) => y.Email.EMail === x.Email))
          .map((x) => ({
            text: `${x.Title} - ${x.Email}`,
            value: x.Id,
            name: x.Title,
