...
class NewTicket extends React.Component<INewTicketProp, INewTicketStates> {
  ...
  private generateTicketId = ({ id }) => {
    try {
      ...
      let customizedId =
        // @ts-ignore
        missingLen > 0 ? "0".repeat(missingLen) + newId : newId.toString();
      return customizedId;
    } catch (error) {
      console.error(error);
    }
  ...
  private saveNewTicket = async () => {
    try {
      ...
      params = createItemParams({
        ...
        listTitle: listTitles.TICKET_INFORMATION_TABLE,
        body: {
          __metadata: { type: "SP.Data.CheckListTransactionListItem" },
          TicketNo: ticketId,
          Title: ticketTitle,
      ...
      params = sendEmailParams({
        ...
        emailBody: {
          ...
          <p>Ticket Due Date: <strong>${this.state.dueDate}</strong></p>
